...
class FeedbackGetByIdTestCase(BaseUnitTest):
    """This class represents the test case for get feedback by id"""

    def list_feedback_invalid_id(self, invalid_id):
        """Test API can get feedback passing invalid id (GET request)"""

        # make a query feedback passing invalid parameter
        res = self.client().get('/feedbacks/'+invalid_id)
        self.assertEqual(res.status_code, 400)
        self.assertIn('The id value is not valid.', str(res.data))      

    def list_feedback_query(self, feedback):
        """Test API can get feedback by id (GET request)"""

        # make a query feedback passing the feedback posted id
        res = self.client().get('/feedbacks/{}'.format(feedback['id']))
        
        # check if the api can get the new feedback posted by the id
    ...
    def test_list_by_id_with_invalid_parameter(self):
        ...
        for id in invalid_ids:
            self.list_feedback_invalid_id(id)


    def test_list_by_id_not_existing_feedback(self):
        """Test API can get feedback passing a not existent id (GET request)"""

        # make a query feedback passing not existing id feedback
        res = self.client().get('/feedbacks/78327fc6-080d-4e45-9ead-3862e30160a8')
    ...
    # create a feedback from user manager to generated submission
        ...
        for feedback in self.feedbacks_list:
            self.list_feedback_query(feedback)
